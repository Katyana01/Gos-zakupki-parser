...
def parse_xml(zip_file, name, region_id):
	try:
		if name.startswith('fcsNotificationZK504') and name.endswith('.xml'):
			xml_file = zip_file.open(name)
			return (5, etree.iterparse(xml_file, tag='{http://zakupki.gov.ru/oos/export/1}epNotificationEZK'), region_id)
		...
		elif name.startswith('epNotificationEZK') and name.endswith('.xml'):
			xml_file = zip_file.open(name)
			return (4, etree.iterparse(xml_file, tag='{http://zakupki.gov.ru/oos/export/1}epNotificationEZK2020'), region_id)
		elif name.startswith('fcsNotificationZK'):
			xml_file = zip_file.open(name)
			return (0, etree.iterparse(xml_file, tag='{http://zakupki.gov.ru/oos/export/1}fcsNotificationZK'), region_id)
		elif name.startswith('epProtocolEZK2020FinalPart') and name.endswith('.xml'):
			xml_file = zip_file.open(name)
			return (6, etree.iterparse(xml_file, tag='{http://zakupki.gov.ru/oos/export/1}epProtocolEZK2020FinalPart'), region_id)
		elif name.startswith('fcsProtocolEZK2') and name.endswith('.xml'):
			xml_file = zip_file.open(name)
			return (7, etree.iterparse(xml_file, tag='{http://zakupki.gov.ru/oos/export/1}epProtocolEZK2'), region_id)
...
def push_to_db(p):
	...
	try:
		if type == 0:
			...
			for event, xml in parsed:
				if event == 'end':
					transform_new_notifications_ep(xml, region_id)
					return
		elif type == 5:
			for event, xml in parsed:
				if event == 'end':
					transform_new_notifications_504(xml, region_id)
					return
		elif type == 6:
			for event, xml in parsed:
				if event == 'end':
					transform_new_protocols_finalpart(xml, region_id)
					return
		elif type == 7:
			for event, xml in parsed:
				if event == 'end':
					transform_new_protocols_ezk2(xml, region_id)
					return
	except Exception:
		return
